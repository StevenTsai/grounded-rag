...
1. **自研合成示例**（`seed_docs.jsonl` / `seed_rules.json` / `eval_set.jsonl` /
   `e2e_eval_set.jsonl` / `binding_eval_set.jsonl`）：虚构药品/标志物组合、虚构
   数值与研究结论，用于演示与机制评测，不含任何真实患者信息或第三方作品原文。

2. **真实场景评测数据**（`real_seed_docs.jsonl` / `real_seed_rules.json` /
   `real_eval_set.jsonl`）：由公开渠道发布/流通的临床指南与文献**整理摘录**而
   成，来源以中国临床肿瘤学会（CSCO）系列诊疗指南为主，另含 NCCN / NICE /
   ESMO / ACR / AIRO 及公开医学平台（如 MedSci）条目——具体来源与版本见各条
   记录 `title` / `source_version` 字段。数据仅用于离线评测与研究对照，已剔除
   患者隐私信息，**不构成任何诊疗建议**。

上述第三方内容的著作权归原作者/机构所有，本仓库不对其主张任何权利；如权利人
认为不应收录，请联系维护者删除。请勿将任何数据用于真实诊疗决策。
