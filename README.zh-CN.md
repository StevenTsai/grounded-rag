...
> ⚠️ `examples/` 中演示/评测数据（`seed_*`、eval sets）为**自研合成示例**；真实场景评测集（`real_seed_*`）整理自公开临床指南/文献（来源见各条记录 `source_version`），仅用于离线评测。以上数据均**不构成真实诊疗建议**。
> 请勿用于任何真实医疗决策。
...
本框架将校验门机制独立开源，附合成示例数据与公开指南整理的真实评测集（`examples/`），供任何垂直领域复用。
