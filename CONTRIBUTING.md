...
| `examples/` | 合成演示数据 + demo（真实评测数据不入库） | **禁止提交第三方受版权内容/真实患者隐私数据**；真实数据仅本地维护并标注来源 |
...
6. [ ] 数据合规：`examples/` 只允许**自研合成示例**；由公开指南/文献整理的
        真实评测数据（含第三方内容）**不得入库**，仅本地维护并标注来源。新增
        依赖先核对许可证并更新 THIRD_PARTY_NOTICES.md。
...
本仓库面向公网开源，**独立成篇**：机制与代码自洽、合成演示数据自足、真实评测数据不入库、不携带任何
