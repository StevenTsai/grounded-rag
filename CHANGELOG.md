...
- 领域适配指南：金融/法律示例因非法 `source_type`（`regulation` 未登记）与错误的扩展点 API 名（`_extract_context` / `guideline_engine`）无法运行
...
- 锚点绑定评测模式 `python -m groundedrag.eval --binding`（确定性、离线、可回归）
...
- 开发状态 classifier `Alpha` → `Beta`；补充 GitHub 镜像 URL；README 社区节统一 Gitee/GitHub 指向
